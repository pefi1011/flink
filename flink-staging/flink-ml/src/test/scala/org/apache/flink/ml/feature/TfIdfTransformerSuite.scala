...
class TfIdfTransformerSuite extends FlatSpec with Matchers with FlinkTestBase {
  ...
  it should "calculate two times one for four words in only one document with stop word parameters" in {
  ...
  it should "calculate valid tfidfs for three poems" in {
    val env = ExecutionEnvironment.getExecutionEnvironment
    val poem1: Seq[String] = env.readTextFile(this.getClass.getClassLoader.getResource("the_passionate_pilgrim.txt").toString).flatMap(s => s.split(" ").toSeq).collect()
    val poem2: Seq[String] = env.readTextFile(this.getClass.getClassLoader.getResource("a_lovers_complaint.txt").toString).flatMap(s => s.split(" ").toSeq).collect()
    val poem3: Seq[String] = env.readTextFile(this.getClass.getClassLoader.getResource("the_phoenix_and_the_turtle.txt").toString).flatMap(s => s.split(" ").toSeq).collect()
    //example results gotten from sklearn with the TfidfVectorizer options smooth_idf=False, norm=None, use_idf=True, sublinear_tf=False
    val expectedResults = env.readTextFile(this.getClass.getClassLoader.getResource("poems_out.txt").toString).map(s => {
      val expr = "\\((.*), (.*)\\), (.*)".r
    ...
    val result = transformer.transform(env.fromCollection(Seq((0, poem1), (1, poem2), (2, poem3)))).collect()
    for ((id, vector) <- result) {
